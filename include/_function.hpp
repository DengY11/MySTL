...
private:
  struct _FuncBase {
    ...
    virtual auto _M_type() -> std::type_info const & = 0;
...
public:
  ...
  noexcept { return _M_base != nullptr; }
  ...
  template<class _Fn> auto target() const
  noexcept -> _Fn * {
  ...
  void swap(function &__that) const
  noexcept { _M_base.swap(__that._M_base); }
};
